local _base = import 'nl2code-base-finetune-bert.libsonnet';
...
function(args) _base(output_from=_output_from, data_path=args.data_path) + {
    ...
    pretrain: {
        pretrained_path: "logdir/bert_large_run_pretrain/bs=6,lr=7.4e-04,bert_lr=3.0e-06,end_lr=0e0,att=1",
        checkpoint_step: 200,
    },
    ...
    model+: {
        ...
        encoder_preproc+: {
            ...
            save_path: data_path + 'nl2code,output_from=%s,fs=%d,emb=bert_large,cvlink' % [_output_from, _fs],
        },
        decoder_preproc+: {
            grammar+: {
